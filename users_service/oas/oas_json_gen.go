...
// Encode implements json.Marshaler.
func (s *WhoamiGetOK) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *WhoamiGetOK) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("user_id")
		s.UserID.Encode(e)
	}
	{
		e.FieldStart("is_root")
		s.IsRoot.Encode(e)
	}
}

var jsonFieldsNameOfWhoamiGetOK = [2]string{
	0: "user_id",
	1: "is_root",
}

// Decode decodes WhoamiGetOK from json.
func (s *WhoamiGetOK) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode WhoamiGetOK to nil")
	}
	var requiredBitSet [1]uint8
	s.setDefaults()
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "is_root":
			requiredBitSet[0] |= 1 << 1
			if err := func() error {
				if err := s.IsRoot.Decode(d); err != nil {
					return err
				}
				return nil
			}(); err != nil {
				return errors.Wrap(err, "decode field \"is_root\"")
			}
		default:
			return d.Skip()
		}
		...
		return errors.Wrap(err, "decode WhoamiGetOK")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfWhoamiGetOK) {
					name = jsonFieldsNameOfWhoamiGetOK[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
// MarshalJSON implements stdjson.Marshaler.
func (s *WhoamiGetOK) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *WhoamiGetOK) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
